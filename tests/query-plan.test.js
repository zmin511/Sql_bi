...
test("each ready selection exposes the public contract", () => { const plan = buildQueryPlan(fixture({ selected: { value: true } })); const selection = plan.selections[0]; for (const key of ["id", "selectionId", "kind", "sourceRowId", "sourceTable", "sourceField", "sourceAlias", "field", "outputAlias", "expression", "status", "reason", "chain"]) assert.ok(Object.hasOwn(selection, key), key); assert.equal(selection.id, "value"); assert.equal(selection.selectionId, "value"); assert.equal(selection.sourceRowId, "value"); assert.equal(selection.sourceTable, "_Document100"); assert.equal(selection.sourceField, "_Fld300"); assert.equal(selection.sourceAlias, "T"); assert.equal(selection.status, "active"); assert.equal(selection.reason, null); });
