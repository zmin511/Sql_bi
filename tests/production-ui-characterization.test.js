...
test("relationship legend exists in markup", () => { const { html } = loadProductionRuntime(); assert.match(html, /id=\"queryGraphLegend\"/); });
test("relationship legend contains textual statuses", () => { const { html } = loadProductionRuntime(); assert.match(html, /В SQL/); assert.match(html, /В плане/); assert.match(html, /Доступна/); assert.match(html, /Предупреждение/); assert.match(html, /Ошибка/); });
test("relationship status normalizer is exposed", () => { const { api } = runtime(); assert.equal(typeof api.normalizeRelationshipStatus, "function"); });
test("unknown relationship status fails safe", () => { const { api } = runtime(); const model = api.buildQueryGraphModel({ rows: rows(), selectedIds: [], metaById: {}, queryPlan: null }); assert.equal(api.normalizeRelationshipStatus({ kind: "edge", status: "nonsense" }, model), "unknown"); });
test("focus state does not change field selection", () => { const value = select(runtime(), "value"); const before = Object.keys(value.api.state.selected); value.api.setQueryGraphFocus("node", "row:value"); assert.deepEqual(Object.keys(value.api.state.selected), before); });
test("focus state is not serialized into project snapshot", () => { const value = runtime(); value.api.setQueryGraphFocus("node", "row:value"); const snapshot = value.api.createProjectSnapshot(); assert.equal("queryGraphFocus" in snapshot, false); });
test("focus reset control exists", () => { const { html } = loadProductionRuntime(); assert.match(html, /id=\"queryGraphResetFocus\"/); });
test("relationship details card container exists", () => { const { html } = loadProductionRuntime(); assert.match(html, /id=\"queryGraphDetails\"/); });
test("details card derives from existing relationship data", () => { const { api } = runtime(); const model = api.buildQueryGraphModel({ rows: rows(), selectedIds: ["value"], metaById: {}, queryPlan: null }); const item = model.edges[0] || model.nodes[0]; assert.ok(item); const details = api.buildRelationshipDetails(item, model); assert.ok(Array.isArray(details)); assert.ok(details.length > 0); });
test("details card does not invent foreign keys", () => { const { api } = runtime(); const edge = { id: "e:test", kind: "edge", sourceId: "row:a", targetId: "row:b", status: "unknown", reason: null, confirmation: null }; const details = api.buildRelationshipDetails(edge, { nodes: [], edges: [edge], activeNodeIds: [], activeEdgeIds: [] }); assert.equal(details.some(row => /внешний ключ|foreign key/i.test(String(row.label) + " " + String(row.value))), false); });
test("graph filter values are preserved", () => { const { html } = loadProductionRuntime(); for (const v of ["all", "active", "sql", "errors"]) assert.match(html, new RegExp(`value=\"${v}\"`)); });
test("graph filter only changes presentation state", () => { const value = runtime(); value.api.state.queryGraph.graphFilter = "sql"; value.api.renderQueryGraph(); assert.equal(value.api.state.queryGraph.graphFilter, "sql"); });
test("graph filter does not change query plan", () => { const value = select(runtime(), "value"); const plan = value.api.state.queryPlan; value.api.state.queryGraph.graphFilter = "errors"; value.api.renderQueryGraph(); assert.equal(value.api.state.queryPlan, plan); });
test("graph filter does not regenerate SQL", () => { const value = select(runtime(), "value"); const sql = value.elements.get("sql").value; value.api.state.queryGraph.graphFilter = "active"; value.api.renderQueryGraph(); assert.equal(value.elements.get("sql").value, sql); });
test("node labels provide title or tooltip fallback", () => { const value = select(runtime(), "value"); value.api.renderQueryGraph(); const html = value.elements.get("queryGraphNodes").innerHTML; assert.ok(/title=/.test(html) || /aria-label=/.test(html)); });
test("focused node has a presentation class", () => { const value = select(runtime(), "value"); value.api.setQueryGraphFocus("node", "row:value"); value.api.renderQueryGraph(); assert.match(value.elements.get("queryGraphNodes").innerHTML, /\bfocus\b/); });
test("unrelated elements can be dimmed in focus mode", () => { const value = select(runtime(), "value"); value.api.setQueryGraphFocus("node", "row:value"); value.api.renderQueryGraph(); assert.match(value.elements.get("queryGraphNodes").innerHTML + value.elements.get("queryGraphSvg").innerHTML, /\bdim\b/); });
test("error and warning statuses have textual labels", () => { const { api } = runtime(); assert.ok(api.relationshipStatusLabel("error")); assert.ok(api.relationshipStatusLabel("warning")); });
test("no relationships empty state exists", () => { const { html } = loadProductionRuntime(); assert.match(html, /id=\"queryGraphEmpty\"/); });
test("no filter matches empty state is rendered", () => { const value = runtime(); value.api.state.queryGraph.graphFilter = "errors"; value.api.renderQueryGraph(); assert.ok(value.elements.get("queryGraphEmpty").textContent.length > 0); });
