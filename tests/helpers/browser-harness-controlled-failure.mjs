...
const temporaryFile = path.join(profile, "owned-temporary-resource.txt");
fs.writeFileSync(temporaryFile, "owned");
...
try {
  ...
  process.stdout.write(`${JSON.stringify({ profile, port, ownedPid: ownedProcess.pid, errorMessage, profileRemoved: !fs.existsSync(profile), markerRemoved: !fs.existsSync(marker), temporaryFileRemoved: !fs.existsSync(temporaryFile), serverClosed: !server.listening })}\n`);
}
