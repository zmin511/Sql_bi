...
const modules = ["sqlIdentifiers.js", "types.js", "tableDetect.js", "relationships.js", "references.js", "selectionContext.js", "casts.js", "dates.js", "filters.js", "queryPlan.js", "queryPlanView.js", "sqlGenerate.js", "project.js"].map(name => `src/core/${name}`);
...
export function buildEmbeddedBlock() {
  ...
  const payload = `(function(){\n${sources.map(item => `// ${item.file}\n${transform(item.source)}`).join("\n")}\nwindow.SQLBICanonicalCore = { buildQueryPlan, buildQueryPlanView, generateSql, normalizeVisualizationSettings, createProjectSnapshot, parseProjectSnapshot, filterFieldType };\n})();\n`;
