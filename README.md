...
SQL BI — portable HTML-инструмент для визуального построения SQL Server-запросов по структуре базы 1С/SQL, выгруженной в XLSX, MXL или XML ValueStorage.
...
- Загружает структуру 1С из `.xlsx`, `.mxl` или XML-контейнера 1С `ValueStorage`.
- Формирует готовый Power Query (M-код) для подключения запроса к SQL Server из Power BI.
- Скачивает сформированный SQL в `.sql`, а Power Query — в `.pq` с UTF-8 BOM.
...
XML-файл может быть сериализованным контейнером 1С `ValueStorage` со сжатым табличным документом. Распаковка выполняется локально средствами современного браузера; файл никуда не отправляется.

## Как построить первый запрос

1. Откройте `index.html`.
2. Загрузите `.xlsx`, `.mxl` или `.xml` со структурой 1С.
...
8. Укажите SQL Server и базу данных, если хотите получить готовое подключение для Power BI.
9. Скопируйте SQL либо скачайте `.sql` или Power Query `.pq`.

## Как использовать Power Query в Power BI

1. В SQL BI укажите имя SQL Server и базы данных.
2. Сформируйте SQL-запрос и нажмите `Скачать Power Query`.
3. Откройте скачанный `.pq` в текстовом редакторе и скопируйте весь M-код.
4. В Power BI откройте `Преобразовать данные` → `Создать источник` → `Пустой запрос`.
5. Откройте `Расширенный редактор`, замените его содержимое и нажмите `Готово`.

M-код создаёт подключение через `Sql.Database()` и выполняет запрос через `Value.NativeQuery()`. Кавычки, переносы строк и символы `#` экранируются по правилам языка M. SQL Server сохраняется в проекте `.sqlbi` вместе с остальными настройками.
...
- Паспорт запроса пока не формируется.
