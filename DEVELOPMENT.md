...
Before any future release run `npm run check:release -- <version>` and stop on a non-zero `$LASTEXITCODE`. Both preflights are read-only. If files are staged, they build an index tree with `git write-tree`, archive its Git blob bytes into a system temporary directory, reject unstaged tracked differences, and validate that exact candidate; the directory is removed in `finally`.
